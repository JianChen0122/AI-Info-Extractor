...
def fix_incomplete_json(json_str):
    """
    Fixes common issues in an incomplete JSON string:
    - Closes unclosed objects `{}` and lists `[]`
    - Removes illegal trailing commas
    """
    # Remove illegal trailing commas before a closing brace or bracket
    ...
    # Auto-fix by adding missing closing braces/brackets
...
def batch_retrieve(output_file_id):
    ...
    try:
        ...
        for i in responses:
            ...
            # check json file: not close (miss }), extra comma before }.
            content_raw_check = fix_incomplete_json(match.group(0))
            try:
                content = json.loads(content_raw_check)
                headings, cells = zip(*dict_extract(content)) # ['Post ID', 'Relevance', ...], ['140', 'No', ...]
            except:
                content_wrong.append((created_time, model_used, content_raw_check))
        ...
        if len(content_wrong) > 0:
            txt_write('wrong.txt', content_wrong)
    except:
...
def update_check(user_name, repository_name, file_path):  
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Latest update: {latest_commit} - {message}")
            ...
            if version_latest != version_pre:                
                ...
                print(f'{file_name} new version found! Do you want to download the latest version?')
                update = human_choose(['yes', 'no'], 1)[0]
                if update == 0:
                    print(f'raising download request through {download_url} ...')
                    webbrowser.open(download_url)
                else:
                    print(f'That\'s okay. You can download it later by yourself through {download_url}')
        else:
...
if __name__ == '__main__':
    # script info
    print('* 本脚本用于调用AI模型结构化提取文本关键信息 （目前仅支持ChatGPT-4o-mini，后续将适配DeepSeek）')
    # check and download the updated progress
    progress = {'AI_Info_Extractor.exe': '2025-02-07', 'README.md': '2025-02-07'}
    for i in progress:
        print(f'* {i}，版本号{progress[i]}')
    print('*** 有限技术支持：sidchen0 @ qq.com ***')
    print('Project address: https://gitee.com/sidchen0122/ai-info-extractor.git')
    # check updates
    for i in progress:
        update_check('sidchen0122', 'ai-info-extractor', i)
